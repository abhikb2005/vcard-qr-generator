...

{
  ...
  assert.match(homepage, /lastFailedStaticGenerationId/);
  ...
  assert.match(homepage, /lastFailedStaticGenerationId === staticGenerationId[\s\S]{0,460}error_stage: 'download'/);
  assert.match(homepage, /lastFailedStaticGenerationId = 0;[\s\S]{0,260}generated_qr_code/);
  assert.match(homepage, /lastFailedStaticGenerationId = generationId;[\s\S]{0,180}error_qr_generation/);
...
for (const file of ['logo-qr-code.html', 'qr-code-with-logo.html']) {
  ...
  assert.match(html, /lastFailedBrandedGenerationId/);
  ...
  assert.match(html, /lastFailedBrandedGenerationId === brandedGenerationId[\s\S]{0,480}error_stage: 'download'/);
  assert.match(html, /lastFailedBrandedGenerationId = 0;[\s\S]{0,280}generated_branded_qr_code/);
  assert.match(html, /lastFailedBrandedGenerationId = generationId;[\s\S]{0,180}error_qr_generation/);
...

{
  ...
  assert.doesNotMatch(dynamicLanding, /adsbygoogle/);
}
...

{
  ...
  assert.match(worker, /function dodoApiKey\(env\)/);
  assert.match(worker, /replace\(\S*\\u0000-\\u001F\\u007F\S*, ''\)/);
